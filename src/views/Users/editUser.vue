            ...
            <div class="w-full mt-5 mr-0 mb-0 ml-0 relative z-10 lg:mt-0 lg:w-5/12">
                <form @submit.prevent="edituser()">
                    <div class="flex flex-col w-full gap-2 items-start justify-start pt-5 pr-6 pb-4 pl-6 bg-white shadow-2xl rounded-xl relative z-10">
                        <p class="w-full text-4xl font-bold text-center leading-snug">Edit User</p>
                        <div class="w-full mt-2 mr-0 mb-0 ml-0 relative space-y-8">
                            ...
                            <div class="relative flex gap-3">
                                <button type="button" @click="edituser()" class="w-full inline-block pt-3 pr-3 pb-3 pl-3 text-xl font-medium text-center text-white bg-indigo-500 rounded-lg transition duration-200 hover:bg-indigo-600 ease">
...
const user_id = router.currentRoute.value.params.slug
console.log(user_id);
console.log(rols_id);
...
function fetchUser(){
     axios
    .get(`https://pm-api.essential.uz/api/users/show/${user_id}`, {
        headers: {
            Authorization: 'Bearer ' + localStorage.getItem('token')
        }
    }).then((res)=>{
        fio.value = res.data.fio;
