...
<section class="flex items-center justify-center h-[80vh]">
      ...
      <div class="w-full mt-5 mr-0 mb-0 ml-0 relative z-10 max-w-2xl lg:mt-0 lg:w-5/12">
       <form @submit.prevent="editRol()">
         <div class="flex flex-col items-start justify-start pt-6 pr-6 pb-6 pl-6 bg-white shadow-2xl rounded-xl
            relative z-10">
          <p class="w-full text-4xl font-bold text-center leading-snug ">ADD User Rol</p>
          <div class="w-full mt-2 mr-0 mb-0 ml-0 relative space-y-8">
            <div class="relative">
              <p class="bg-white pt-0 pr-2 pb-0 pl-2 -mt-3 mr-0 mb-0 ml-2 font-medium text-gray-600
                  absolute">Name</p>
              <input v-model="name" placeholder="John" type="text" class="border placeholder-gray-400 focus:outline-none
            ...
            <div class="relative">
              <p class="bg-white pt-0 pr-2 pb-0 pl-2 -mt-3 mr-0 mb-0 ml-2 font-medium text-gray-600
                  absolute">Key</p>
              <input v-model="key" placeholder="Key" type="text" class="border placeholder-gray-400 focus:outline-none
                  focus:border-black w-full pt-4 pr-4 pb-4 pl-4 mt-2 mr-0 mb-0 ml-0 text-base block bg-white
                  border-gray-300 rounded-md"/>
            </div>
            <div class="relative">
...
const rol_id=ref(router.currentRoute.value.params.id);
console.log(rol_id.value);
...
function editRol(){
     axios
        .post(
            `https://pm-api.essential.uz/api/user-roles/${rol_id.value}/update`,
            {
                name: name.value,
                key: key.value
            },
...
function fetchRol() {
    axios
    .get(`https://pm-api.essential.uz/api/user-roles/show/${rol_id.value}`, {
        headers: {
            Authorization: 'Bearer ' + localStorage.getItem('token')
        }
    }).then((res) => {
