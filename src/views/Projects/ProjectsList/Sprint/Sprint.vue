        ...
        <Dialog v-model:visible="deletModal" header="Delet Project" :style="{ width: '25rem' }">
            <div class="p-2 pt-0 text-center">
                ...
                <button @click="deletProject" class="text-white bg-red-600 hover:bg-red-300 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">
                    ...
                    <span :class="editisloading ? 'hidden' : 'block text-xl'">O'chirish</span>
                </button>
...
const deletProject = () => {
    ...
    axios
        .delete(`https://pm-api.essential.uz/api/sprints/${eId.value}/delete`, {
            headers: {
...
function fetchData() {
    axios
        .get(`https://pm-api.essential.uz/api/sprints?milestone_id=${milestone_id}&per_page=10`, {
            headers: {
                Authorization: 'Bearer ' + localStorage.getItem('token')
            }
        })
...
const editProject = (id) => {
    ...
    axios
        .post(
            `https://pm-api.essential.uz/api/sprints/${editId.value}/update`,
            {
                name: editName.value,
...
const addProject = () => {
    router.push(`/projects_list/${project_id}/melistone/${milestone_id}/addSprint`);
};
